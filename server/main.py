...
def make_cards(results, limit=20, skip_id=None):
    ...
    Normalizes OMDB's raw 'imdbID' field into our own 'omdb_id' key
    so nothing downstream (or in the frontend) has to touch 'imdb' at all.
    """
    cards = []
    for m in (results or [])[:limit]:
        raw_id = m.get("imdbID", "")
        if skip_id and raw_id == skip_id:
            continue
...
async def get_details(omdb_id) -> MovieDetails:
    """Get full details of one movie by its OMDB id."""
    data = await omdb_call({"i": omdb_id, "plot": "full"})
    ...
    return MovieDetails(
        omdb_id=data.get("imdbID", omdb_id),
...
async def keyword_search(query: str = Query(..., min_length=1), page: int = 1):
    """Search movies by keyword. Returns normalized cards (omdb_id, not imdbID)."""
    data = await search_omdb(query, page)
    return {"Search": [c.model_dump() for c in make_cards(data.get("Search", []), limit=len(data.get("Search", [])))]}


@app.get("/movie/id/{omdb_id}", response_model=MovieDetails)
async def movie_details(omdb_id: str):
    """Get details of one movie."""
    return await get_details(omdb_id)
...
async def search_bundle(query: str, tfidf_top_n: int = 12, genre_limit: int = 12):
    ...
    if details.genres:
        ...
        genre_recs = make_cards(gdata.get("Search", []), genre_limit, skip_id=details.omdb_id)
